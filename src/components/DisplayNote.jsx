...
function DisplayNote(props) {
    return (
        <>
            <div style={{ background: props.color }} className="border rounded-lg px-5 py-3 w-[80vw] sm:w-[570px] my-8 mx-auto hover:shadow-md">
                {props.title ? <h1 className="font-serif tracking-wide font-semibold pb-2">{props.title}</h1> : null}
                {props.description ? <pre className="text-sm pb-2 font-serif tracking-wide">{props.description}</pre> : null}
